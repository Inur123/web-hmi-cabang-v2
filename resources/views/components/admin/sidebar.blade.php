<div :class="sidebarOpen ? 'w-64' : 'w-20'"
    ...
    <nav class="p-3 space-y-2 overflow-y-auto h-[calc(100vh-77px)]">
        ...
        <!-- Dashboard -->
        <button type="button"
            @click="Livewire.navigate('{{ route('admin.dashboard') }}')"
        ...
        <div x-data="{ open: {{ request()->routeIs('admin.categories') ? 'true' : 'false' }} }">
            <button type="button" @click="sidebarOpen && (open = !open)"
                class="w-full flex items-center rounded-lg transition cursor-pointer {{ request()->routeIs('admin.categories') ? 'bg-teal-600 text-white' : 'text-gray-700 hover:bg-gray-100' }}"
