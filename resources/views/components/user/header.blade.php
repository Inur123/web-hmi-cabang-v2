    ...
    text-transparent bg-clip-text">
        ...
        <div class="flex space-x-8 flex-1 justify-center">
                ...
                <div x-show="openBlog" x-cloak @click.away="openBlog = false"
                    ...
                    <div class="overflow-y-auto max-h-80">
                        <a href="{{ route('blog') }}" wire:navigate
                            x-bind:class="isActive('{{ route('blog') }}') ? 'bg-green-50 dark:bg-green-900/30' : ''"
                        ...
                        @foreach ($allCategories as $category)
                            <a href="{{ route('categories.show', $category->slug) }}" wire:navigate
                                x-bind:class="isActive('{{ route('categories.show', $category->slug) }}') ?
                                ...
                                <div class="flex items-center justify-between gap-2">
                                    ...
                                    <span
                                        class="text-xs bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 px-1.5 py-0.5 rounded-full font-medium flex-shrink-0">
            ...
            <div class="relative" x-data="{ openProfile: false }">
                <span @click="openProfile = !openProfile"
                    x-bind:class="isProfileActive() ? 'text-green-600 dark:text-green-400 font-bold' :
                ...
                <div x-show="openProfile" x-cloak @click.away="openProfile = false"
                    ...
                    <a href="{{ route('profile.sejarah') }}" wire:navigate
                        x-bind:class="isActive('{{ route('profile.sejarah') }}') ? 'bg-green-50 dark:bg-green-900/30' : ''"
                    ...
                    <a href="{{ route('profile.kepengurusan') }}" wire:navigate
                        x-bind:class="isActive('{{ route('profile.kepengurusan') }}') ? 'bg-green-50 dark:bg-green-900/30' : ''"
        ...
        <div class="flex items-center justify-end flex-1">
            ...
            <button @click="toggleDarkMode()"
                ...
                <!-- SUN ICON (untuk mode terang / light mode) -->
                ...
                <!-- MOON ICON (untuk mode gelap / dark mode) -->
    ...
    <div x-show="mobileMenuOpen" x-cloak @click.away="mobileMenuOpen = false"
        ...
        <div class="border-b border-gray-200 dark:border-gray-700" x-data="{ openBlogMobile: false }">
            <span @click="openBlogMobile = !openBlogMobile"
                ...
                Blog
                <i :class="{ 'rotate-180': openBlogMobile }"
                    class="fas fa-chevron-down text-xs transition-transform duration-200"></i>
            </span>
            <div x-show="openBlogMobile" x-cloak x-transition:enter="transition ease-out duration-200"
                ...
                <a href="{{ route('blog') }}" wire:navigate
                    x-bind:class="isActive('{{ route('blog') }}') ? 'bg-green-100 dark:bg-green-800/50' : ''"
        ...
        <div class="border-b border-gray-200 dark:border-gray-700" x-data="{ openProfileMobile: false }">
            <span @click="openProfileMobile = !openProfileMobile"
                ...
                class="flex items-center justify-between p-4 cursor-pointer transition-colors duration-200">
                Profile
                <i :class="{ 'rotate-180': openProfileMobile }"
                    class="fas fa-chevron-down text-xs transition-transform duration-200"></i>
            </span>
            <div x-show="openProfileMobile" x-cloak x-transition:enter="transition ease-out duration-200"
                x-transition:enter-start="opacity-0 max-h-0" x-transition:enter-end="opacity-100 max-h-96"
                x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 max-h-96"
                x-transition:leave-end="opacity-0 max-h-0"
                class="bg-gray-50 dark:bg-gray-700/50 max-h-64 overflow-y-auto">
                <a href="{{ route('profile.sejarah') }}" wire:navigate
                    x-bind:class="isActive('{{ route('profile.sejarah') }}') ? 'bg-green-100 dark:bg-green-800/50' : ''"
                    class="block px-8 py-3 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600 font-semibold border-b border-gray-200 dark:border-gray-600 transition-colors duration-200">
                    Sejarah
                </a>
                <a href="{{ route('profile.kepengurusan') }}" wire:navigate
                    x-bind:class="isActive('{{ route('profile.kepengurusan') }}') ? 'bg-green-100 dark:bg-green-800/50' : ''"
